...
# input discount rate
discount = pd.read_sql_table(table_name='zdiscount', con=engine)

# data4['Date'] = data4['Date'].map(lambda x: pd.to_datetime(x))
data5 = pd.merge(data4, discount, how='left', on='Date')
data5.drop(columns=['index_x'], inplace=True)

data5.to_sql(name='zcombine', con=engine, if_exists='replace')
